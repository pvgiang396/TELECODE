...
- `setup.sh` tự tạo shortcut Desktop mở `http://localhost:8443` (`~/Desktop/code-server.desktop` trên Linux — dùng icon PWA có sẵn của code-server; `~/Desktop/*.command` trên macOS) — dùng VS Code như app desktop ngay trên máy chạy code-server, không cần qua tunnel/Telegram. `install.ps1` (Windows) tạo tương ứng 1 file `.url` trên Desktop Windows (không phải trong WSL) nhờ WSL2 tự forward `localhost` sang host.
