...
- **Tự đóng terminal sau khi cài xong**: `setup.sh` lấy `$PPID` lúc mở wizard (PID shell cha — do `install.sh` dùng `exec bash setup.sh` nên PID không đổi xuyên suốt, `$PPID` chính là shell tương tác gõ lệnh), truyền làm argv thứ 3 cho `wizard.py`. `wizard.py` lưu PID này vào `wizard-answers.json` dưới key `_callerPid` **ở phía server** (từ argv, không lấy field cùng tên nếu client gửi lên trong form — tránh giả mạo). Bước cuối cùng của `setup.sh` (nhánh `TELECODE_APPLYING=1`, sau khi bot chạy xong, trước dòng "Setup xong!") đọc `_callerPid` và `kill` nếu process còn sống — không hỏi xác nhận, đóng terminal luôn. Nhánh headless không có PID này (không qua wizard nên không áp dụng).
...
- `patch_code_server_favicon()` trong `setup.sh` ghi đè trực tiếp file favicon/PWA icon của code-server (`{root}/src/browser/media/favicon.ico`, `favicon-dark-support.svg`, `pwa-icon-{192,512}.png`, `pwa-icon-maskable-{192,512}.png` nếu tồn tại) bằng icon riêng từ `assets/` (`icon.ico`, `favicon.svg` — SVG bọc `icon.png` dạng base64 data URI, `pwa-icon-{192,512}.png` resize từ `icon.png`) — icon taskbar khi mở qua Chrome `--app=` lấy từ favicon của trang, không phải từ `Icon=` trong `.desktop`/`.lnk`, nên phải ghi đè file thật thay vì chỉ patch HTML. Không đụng `login.html`/`workbench.html` — chỉ thay file ảnh, không có rủi ro như patch HTML/script. Idempotent qua `cmp` so nội dung `favicon.ico` với bản trong `assets/` trước khi ghi (tránh hỏi `sudo` lại mỗi lần chạy `setup.sh` nếu đã khớp). Toàn bộ 6 lệnh `cp` gộp vào **1 lệnh `sudo bash -c "..."` duy nhất** — chạy nền qua `nohup` (không có TTY thật) khiến sudo không cache được xác thực giữa các lần gọi riêng lẻ, 6 lệnh `sudo cp` tách rời từng cái sẽ hỏi lại mật khẩu 6 lần liên tiếp (bug thật đã gặp).
