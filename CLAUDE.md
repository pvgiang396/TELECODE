...
- **Bước 9 (chạy `bot.py`) dùng `setsid nohup ... &` (không chỉ `nohup`)** — bug thật đã gặp: chỉ `nohup` không tách session, nếu terminal/session cha bị đóng đột ngột (không phải thoát sạch), cả process group kể cả tiến trình `nohup` con vẫn có thể bị kill theo (nohup chỉ chặn tín hiệu SIGHUP, không phải mọi tín hiệu terminate). Triệu chứng quan sát được: bot chết lặng lẽ giữa phiên, không có traceback trong `logs/bot.log`, không có OOM-killer log trong `syslog`/kernel — chỉ phát hiện được qua `ps -p $(cat .run/bot.pid)`. `setsid` tách hẳn session, tiến trình con có cha là `init`/`systemd --user`, sống sót qua việc đóng terminal khởi chạy nó. Đã verify `$!` (PID capture) vẫn đúng qua `setsid` (util-linux ≥2.28 exec thẳng không fork thêm khi có thể).
