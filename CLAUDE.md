...
- **Shortcut Desktop "Telecode" tự khởi động lại code-server/bot/Tailscale nếu đang tắt trước khi mở app** (`scripts/launch.sh`, gọi bởi `Exec=` của `telecode.desktop`/`Telecode.command` — setup.sh bước 1cb sinh ra) — xử lý đúng tình huống thật đã gặp: Tailscale tự đăng xuất ("Stopped; run 'tailscale up' to log in") sau khi máy sleep/đổi mạng, `tailscaled` daemon + code-server + bot vẫn chạy bình thường nhưng mini app trên điện thoại không vào được (Funnel không còn public) dù mở trên máy tính vẫn OK — trước đây phải tự debug tay (`tailscale status`, `tailscale up ...`). `launch.sh`: (1) start lại `code-server`/`telecode-bot` qua `systemctl --user` nếu không active (KHÔNG tạo/ghi đè unit, chỉ start service đã có sẵn từ lần chạy `setup.sh` trước); (2) kiểm tra `tailscale status --json`'s `BackendState` khác `"Running"` thì chạy lại `tailscale up --hostname=$(hostname) --operator=$(whoami)` (kèm `systemctl start tailscaled` trước nếu daemon cũng tắt hẳn) qua **`pkexec`** (không phải `sudo`/cmdctl — cmdctl chỉ dành cho AI agent chạy lệnh từ xa qua chat, còn shortcut chạy trực tiếp trong phiên đồ hoạ của user nên `pkexec` tự hiện popup xin mật khẩu, không cần TTY/terminal); (3) mở trình duyệt `--app=http://localhost:8443` như cũ. macOS/máy không có `systemd --user` → fallback `nohup` (đọc lại `CS_ENV_FILE`/workspace mặc định đã ghi từ lần `setup.sh` trước, không hỏi lại gì). **Windows (WSL) chưa áp dụng** — `.lnk` do `install.ps1` sinh vẫn mở thẳng `msedge.exe` như cũ, không gọi qua `launch.sh` (cần wrapper `.bat`/PowerShell gọi `wsl.exe bash -lc launch.sh` để làm tương tự, chưa triển khai).
