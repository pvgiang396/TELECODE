...
Điều khiển VS Code trên máy tính của bạn từ điện thoại hoặc desktop app riêng. Tích hợp Claude for VS Code để giao việc coding từ xa.

## ✨ Tính năng

- 📱 Mở VS Code từ app Telecode (desktop Tauri hoặc Android)
- 🤖 Tích hợp Claude for VS Code Extension
- 🌐 Truy cập từ bất kỳ đâu qua Tailscale Funnel
...
**Trên máy có giao diện (desktop Linux/macOS, hoặc Windows qua WSL)**: `setup.sh` tự mở 1 trang web (`http://127.0.0.1:8899`) cho bạn điền cấu hình bằng radio/input thay vì hỏi qua terminal — điền xong bấm "Lưu cấu hình", việc cài đặt thật (code-server, Tailscale, tunnel) chuyển sang chạy nền, terminal trả quyền điều khiển lại ngay, có thể đóng terminal an toàn. Chạy lại đúng lệnh Quick Start bất cứ lúc nào để cập nhật/khởi động lại — form tự hiện đúng bước cần hỏi dựa theo trạng thái máy hiện tại (đã cài gì, đang chạy gì), không hỏi lại token/mật khẩu nếu đã cấu hình (chỉ hiện sẵn giá trị cũ).
...
`setup.sh` làm toàn bộ các việc: mở wizard web thu thập cấu hình (hoặc hỏi qua terminal nếu headless), cài code-server + Tailscale, thêm icon hiện/ẩn mật khẩu + chặn F12/chuột phải vào trang login code-server, tạo password, chạy code-server nền, tạo shortcut Desktop, mở Funnel, tạo `config.yaml` từ `config.example.yaml`, cài dependency Python (venv riêng). Chạy lại `bash setup.sh` bất cứ lúc nào — idempotent, không tạo tiến trình trùng lặp.

### Trên điện thoại (app Telecode Android)

- Cài app Telecode (`.apk`, xem mục build ở `CLAUDE.md`) hoặc mở app desktop Tauri.
- Bấm icon ⚙️ (bánh răng) ở góc trên.
- Dán URL Tailscale Funnel hiển thị sau khi `setup.sh` chạy xong (dạng `https://<machine>.<tailnet>.ts.net`).
- App tự mở VS Code (code-server) qua URL đó — đăng nhập bằng mật khẩu đã đặt ở bước cấu hình `config.yaml`.

### Trên máy tính (app Telecode desktop, dùng như VS Code desktop)

App Telecode (Tauri, xem `CLAUDE.md`) tự mở 1 cửa sổ trỏ thẳng `http://localhost:8443` — nhanh hơn nhiều so với qua Funnel vì không qua tunnel. Đây là **cùng 1 phiên làm việc** với bản mở từ điện thoại (cùng file, cùng extension host) — mở song song ở cả 2 nơi vẫn an toàn.
...
├── src-tauri/             # App desktop/Android Tauri (xem CLAUDE.md)
...
# Kiểm tra Tailscale Funnel
tailscale funnel status
```

### Đăng nhập đúng mật khẩu nhưng bị đá về lại màn login
- Kiểm tra `config.yaml`/`~/.config/code-server/config.yaml` có cùng 1 password và code-server đã được **restart** sau khi đổi password (code-server chỉ đọc config lúc khởi động).
...
2. Xem logs: `journalctl -u code-server -f`
